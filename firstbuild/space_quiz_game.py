"""
Patched space_quiz_game.py - with visual bullet travel, muzzle flash, and hit/fire sounds.
Drop this file and the provided fire.wav / hit.wav into the same folder as questions.csv and run:
    python space_quiz_game.py
"""
...
PLAYER_SPEED = 6
BULLET_SPEED = -8  # vertical speed (negative = up)
ENEMY_ROWS = 3
ENEMY_COLS = 6
ENEMY_PADDING = 10
LIVES = 3
...
def main():
    ...
    pygame.display.set_caption("Study Invaders - Answer to Fire!")
    ...
    while running:
                    ...
                    for i, r in enumerate(rects):
                        if r.collidepoint(mx, my):
                            if i == correct_choice_index:
                                # correct -> choose a target and fire a bullet aimed at it
                                if len(enemies.sprites()) > 0:
                                    ...
                                    # compute time to reach target vertically
                                    dy = player.rect.top - target.rect.centery
                                    if dy <= 0:
                                        dy = 1
                                    time_to_reach = abs(dy / BULLET_SPEED)
                                    dx = target.rect.centerx - player.rect.centerx
                                    vx = dx / time_to_reach
                                    # create bullet with vx so it travels toward the target
                                    # create bullet with vx so it travels toward the target
                                    b = Bullet(
                                        player.rect.centerx,
                                        player.rect.top,
                                    ...
                                    # tag this bullet so we know which enemy it was aimed at
                                    b.target = target
                                    bullets.add(b)
        ...
        if state == "playing":
            ...
            # check collisions: bullet hits enemy (normal collisions)
            ...
            # If no direct collision occurred for the pending target, force the hit when the aimed bullet reaches/passes the target
            if pending_target is not None:
                # iterate a copy of bullets (we may remove one inside)
                for b in list(bullets):
                    # only consider bullets that were tagged with the intended target
                    if getattr(b, "target", None) is pending_target:
                        # if the bullet collides (safety) OR the bullet's top has passed the bottom of the target (i.e., reached/passed)
                        if (
                            b.rect.colliderect(pending_target.rect)
                            or b.rect.top <= pending_target.rect.bottom
                        ):
                            ...
                            if hit_snd:
                                try:
                                    ...
                                    pass
                            # clear pending target and advance
                            pending_target = None
                            if not enemies:
                                spawn_enemies()
                            load_next_question()
