...
def run_settings_screen(screen, settings: Settings):
    ...
    opts = [
        ...
        ("Time between Qs", ["unlimited", "10s", "30s", "60s", "120s"], "unlimited" if settings.time_between_questions is None else f"{int(settings.time_between_questions)}s"),
        ("Total time", ["unlimited", "5m", "15m", "30m"], "unlimited" if settings.total_time is None else f"{int(settings.total_time)}s"),
        ("Lives", ["3", "5", "10", "unlimited"], str(settings.lives) if settings.lives is not None else "unlimited"),
    ...
    while running:
                ...
                if save.collidepoint(mx,my):
                    ...
                    for label, choices, cur in opts:
                        if label == "Question order":
                            ...
                            settings.time_between_questions = None if cur == "unlimited" else int(cur.rstrip('s'))
                        elif label == "Total time":
                            settings.total_time = None if cur == "unlimited" else (int(cur.rstrip('m'))*60 if cur.endswith('m') else int(cur.rstrip('s')))
                        elif label == "Lives":
        ...
        screen.blit(font.render("Back", True, (255,255,255)), (back.x+38, back.y+12))
        save = utils.button_rect(700, 520, w=140, h=44)
        pygame.draw.rect(screen, (50,150,80), save, border_radius=8)
        screen.blit(font.render("Save", True, (255,255,255)), (save.x+44, save.y+12))
...
def run_menu():
    ...
    while True:
        ...
        screen.blit(font.render("Settings", True, (255,255,255)), (settings_rect.x+14, settings_rect.y+14))
